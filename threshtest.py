...
threshs = np.arange(0,21,1)
#mrn/grn thresh tests
mrnNames = [[] for t in threshs]
grnNames = [[] for t in threshs]
nonmrnNames = [[] for t in threshs]
nongrnNames = [[] for t in threshs]
...
for i, thresh in enumerate(threshs):
    for cell, targets in merged.iterrows():
        ...
        grn = targets['GRN']
        if mrn > thresh:
            mrnNames[i].append(cell)
        if mrn <= thresh:
            nonmrnNames[i].append(cell)
        if grn > thresh:
            grnNames[i].append(cell)
        if grn <= thresh:
            nongrnNames[i].append(cell)
            
mrntoPlot = [np.size(sublist) for sublist in mrnNames]
grntoPlot = [np.size(sublist) for sublist in grnNames]

fig, [mrnax, grnax] = plt.subplots(2, 1, figsize=(8,8))
fig.suptitle('Threshold for classifying a cell as GRN/MRN projecting')
mrnax.plot(threshs, mrntoPlot, color='green')
mrnax.set_xlabel('threshold (# of endpoints)')
mrnax.set_ylabel('# of cells')
mrnax.set_title('MRN')
grnax.plot(threshs, grntoPlot, color='purple')
grnax.set_xlabel('threshold (# of endpoints)')
grnax.set_ylabel('# of cells')
grnax.set_title('GRN')
fig.tight_layout()
#premotor thresholding tests
# =============================================================================
